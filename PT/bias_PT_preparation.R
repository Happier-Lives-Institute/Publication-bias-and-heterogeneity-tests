...
dat %<>% mutate(
    SE_corrected = if_else(is.na(n.t), sqrt(((n/2)*(n/2)) / ((n/2)*(n/2))),
                           sqrt((n.t   + n.c) / (n.t  * n.c))),
    SE_corrected_sq = SE_corrected^2
) #%>%
#select(SE, SE_corrected) ##They seem in the ballpark of one another.
...
dat %<>% group_by(Authors) %>%
    mutate(
        FUPisFirst = if_else(FollowUp == min(FollowUp), 1 ,0)
    ) %>% ungroup()

# Does this look right?
dat %>% group_by(Authors, FollowUp, FUPisFirst) %>% summarise(n = n()) %>% ungroup()
...
dat_RE_noout <- dat %>% filter(Authors %ni% m.RE.outliers$out.study.random)
...
dat_FE_noout <- dat %>% filter(Authors %ni% m.FE.outliers$out.study.fixed)
